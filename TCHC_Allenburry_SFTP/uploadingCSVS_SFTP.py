...
def CreateCSVs(date2TryE,file2Try):
	myQueryHeaders="SELECT 'TimeStamping','SN','Load label','Reading','Utility','Unit' FROM Reg5MinReadings UNION "
	myQueryE="SELECT TimeStamping,SN,MeterParam,value,'E','kwh' FROM Reg5MinReadings WHERE DATE(TimeStamping)='%s' "%(date2TryE)
	#myQueryW="SELECT TimeStamping,SN,MeterParam,value,'W','m3' FROM Reg5MinW WHERE DATE(TimeStamping)='%s' UNION "%(date2TryW)
	#myQueryG="SELECT TimeStamping,SN,MeterParam,value,'G','m3' FROM Reg5MinG WHERE DATE(TimeStamping)='%s'"%(date2TryG)
	myQueryCSV="INTO OUTFILE '%s'  FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n'"%(file2Try)
	myQueryTotal=myQueryHeaders+myQueryE+myQueryCSV
	print(myQueryTotal)
	#print("\n")
...
for apple in datestoUpload:
	...
	dataBaseArray=['Reg5Min','Reg5Min', 'Reg5Min']
	lastDayOfData=[]
	...
	lastDayE=lastDayOfData[0][0]["Reg5Min"]
	#lastDayW=lastDayOfData[1][0]["Reg5MinW"]
	#lastDayG=lastDayOfData[2][0]["Reg5MinG"]

	if lastDayE != None:
		fileCreated=CreateCSVs(lastDayE,fileName)
		try:
